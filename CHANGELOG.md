...
101. Updated Interviews tab resume modal buttons to match lobby states (visual-v3) — live event candidates (Dallas/Chicago) show Message + Download matching lobby Waiting to Interview live, non-live event candidates (Atlanta) and Upcoming Active show Reschedule + Decline + Download matching lobby confirmed interviews active state
...
122. Styled "Who needs to receive updates?" subtitle as helper text (visual-v3, edit-post-v3, setup-flow-v3) — 13px, weight 400, #767676, 4px gap from heading. Consistent across all modals and setup steps
123. Created lobby-loader.html — "Loading event lobby..." spinner with 3s delay, fade-out, green checkmark success state linking to lobby-v3
124. Added "Lobby" link to Loaders dev toggle in dashboard header (visual-v3)
125. Removed JobFairX logo from top header bar and compacted all dev toggles (smaller font, tighter padding) to fit all controls (visual-v3)
126. **DEV SPEC — Account owner interviewer tooltip**: When the account owner checks "I will be interviewing" in Interview Settings, the dashboard interviewer count increases (e.g., "1 interviewer") but seat count stays at 0. Add a hover tooltip on the interviewer count link in the dashboard table that says: "Includes account owner. Doesn't count toward interviewer seats." This tooltip should only appear when the account owner has opted in to interview. The tooltip resolves the visual mismatch between interviewer count and seats used without adding clutter to the dashboard row
127. Redesigned message modal to Indeed-style bottom-right message widget (lobby-v3) — fixed position 380×480px, dark header with candidate name/job/event, minimize (−) collapses to header bar, close (×) removes entirely, one widget at a time (swaps on new Message click), no overlay so lobby remains fully interactive, persists when interview screen opens in separate window
128. Converted Interviews tab message modal to bottom-right widget (visual-v3) — replaced centered iv-msg-modal overlay with same Indeed-style widget pattern as lobby-v3, dark header with candidate name/job/event, minimize and close controls, no overlay, page stays interactive
...
133. Reschedule flow state changes (lobby-v3) — after employer proposes new interview times via the schedule modal, the candidate row updates in-place: time column shows proposed time(s) with hover tooltip for multiples, action column shows "Proposed · awaiting candidate" label with proposed time(s) on a compact 2-line layout and a "Cancel" link. Cancel opens a confirmation modal (matching decline modal pattern) with "Don't show this message when I cancel" checkbox, "Keep proposal" dismiss, and red "Cancel" confirm. Confirming restores the row to original Accept/Decline/Reschedule state. Pre-populated 4 demo rows (2 single-time, 2 multi-time)
134. Resume requirement section (edit-post-v3) — added standalone "Resume requirement" section card between Interviewing For and Interview Details. Simple toggle with "Require candidates to upload a resume" label showing Yes (blue) / No (gray) status. Marked as required field with red asterisk. No expand/collapse — always visible.
135. Resume requirement toggle in setup flow (setup-flow-v3) — added editable toggle in Step 5 (Interview Settings) after interviews-per-slot field, matching the is-field-group pattern with question label "Require candidates to upload a resume?" and toggle switch. Added read-only review display in Step 7 with pencil edit button linking back to Step 5. Review text dynamically reads "Resume is required" or "Resume is not required" based on toggle state via populateStep7Review().
136. Resume requirement review pencil opens inline modal instead of navigating back to Step 5 (setup-flow-v3) — compact modal with toggle and Save/Cancel buttons, syncs both the review display and the Step 5 toggle when saved
137. Resume modal matches proposed-reschedule state (lobby-v3) — when a candidate has a pending reschedule proposal ("Proposed · awaiting candidate"), the resume modal now shows "Proposed · awaiting candidate" label with proposed time(s) and a "Cancel proposal" button instead of Accept/Decline/Reschedule. Cancel triggers the same cancel-proposal confirmation modal as the lobby row.
