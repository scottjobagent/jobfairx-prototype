...
116. Redesigned Interviews header and filter (visual-v3) — fixed filter width to 340px so it never jumps when switching events, removed all Live badges from filter button and dropdown items, moved Live indicator (green pulsing dot + "Live" label) below Go to Lobby button, clean dropdown is now just event names, header reads: Interviews [Event ∨] or Interviews [Event ∨] [Go to Lobby / · Live]
