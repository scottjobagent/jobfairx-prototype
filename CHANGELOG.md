...
116. Simplified Interviews header layout (visual-v3) — removed Live badge from filter button and header row entirely, removed min-width from filter so it auto-sizes to content, Go to Lobby button is now the sole live indicator (appears only when a live event is selected), dropdown retains green live badges on Dallas/Chicago for developer reference
