...
116. Redesigned Interviews header layout (visual-v3) — moved Live badge out of filter button into standalone element between filter and Go to Lobby, added min-width 280px to filter button for consistent sizing, removed margin-left:auto from Go to Lobby so it groups tightly with filter, header now reads: Interviews [Event ∨] · Live [Go to Lobby]
