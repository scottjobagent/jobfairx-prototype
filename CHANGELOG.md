...
88. Moved Interviews tab state toggles from inline content to top header bar (visual-v3) — removed per-section "State:" toggles from Upcoming/Pending/Past empty and active views, added consolidated "Interviews tab states:" toggle group to header with Upcoming (Empty/Active/Live), Pending (Empty/Active), Past (Empty/Active) sub-labels, auto-shows when Interviews nav is active
